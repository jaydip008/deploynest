(function ($) {
	...
	if ($('.our-features-gold').length) {
		const testimonial_slider = new Swiper('.our-features-gold .swiper', {
			slidesPerView : 1,
			speed: 1000,
			spaceBetween: 30,
			loop: true,
			...
			pagination: {
				el: '.our-features-gold-pagination',
				clickable: true,
			},
			breakpoints: {
				768:{
...
document.querySelectorAll('.export-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      // agar aa content already open hoy to close kari do
      if (target.classList.contains('show')) {
        target.classList.remove('show');
        return;
      }

      // pehla badha close karo
      document.querySelectorAll('.export-content').forEach(c => {
        c.classList.remove('show');
      });

      // pachi current open karo
